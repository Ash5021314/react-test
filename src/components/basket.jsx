...
export default function Basket() {
    ...
    function removeElementFromBasket(id) {
        let basketCurrentItem = cartProduct.filter((item) => item.id === id);
        let basketItem = cartProduct.filter((item) => item.id !== id);
        basketTotalCount = basketTotalCount - basketCurrentItem[0].quantity

        setCartProduct(basketItem)
        setTotalItem(basketTotalCount)
        setItemInLocalstorage('cart', JSON.stringify(basketItem))
                ...
                {cartProduct.map((item) => (
                    <Col key={item.id}>
                        <Card style={{width: '18rem', height: '500px', padding: '20px'}}>
                            <FaWindowClose onClick={() => removeElementFromBasket(item.id)}
